...
import { Heart, Star, Sparkles, MessageCircle, Users, HandHeart } from 'lucide-react';
...
const HomePage: React.FC = () => {
  ...
  if (!files) return; // Guard against null
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const services: Service[] = [
    { icon: '👕', title: 'Wearable Art', description: 'Elevate your wardrobe with hand-embroidered designs on clothing and accessories. Whether you provide the item or choose one from my collection, each piece is crafted with care and attention to detail.', color: 'from-[#bba987]/30 to-[#bba987]/50' },
    { icon: '🎁', title: 'Personalised Gifts', description: 'Celebrate life’s milestones with thoughtfully embroidered keepsakes. Each gift is customised to reflect your story, making it a meaningful treasure for years to come.', color: 'from-[#bba987]/40 to-[#bba987]/60' },
    { icon: '🪡', title: 'Custom Home Embroidery', description: 'Add warmth and personality to your home with embroidered blankets, cushions, and décor. Each piece is carefully stitched to bring comfort and style to any space.', color: 'from-[#bba987]/35 to-[#bba987]/55' },
    { icon: '✨', title: 'One-of-a-Kind Creations', description: 'Have a unique idea or design in mind? Let’s collaborate to create a completely custom, hand-embroidered piece that reflects your vision and individuality.', color: 'from-[#bba987]/45 to-[#bba987]/65' }
  ];
        ...
        <div className="relative z-10">
          <div className="w-48 h-48 sm:w-64 sm:h-64 lg:w-80 lg:h-80 mx-auto mb-8 relative group">
            <div className="relative w-full h-full bg-white/90 backdrop-blur-lg rounded-full flex items-center justify-center shadow-2xl border border-[#bba987]/30 group-hover:scale-110 transition-transform duration-500">
              <Avatar variant="square" src={logo} alt="Cosy Corner Embroidery Logo" className="w-[96%] h-auto rounded-full" />
            </div>
          </div>
      ...
      <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-[#bba987]/30 overflow-hidden p-8 space-y-6">
        <div className="grid md:grid-cols-2 gap-6">
          <input
            ...
            className="p-4 rounded-xl border border-[#bba987]/50 w-full"
            required
          />
          <input
            type="text"
        ...
        <textarea
          ...
          onChange={handleInputChange}
          className="p-4 rounded-xl border border-[#bba987]/50 w-full h-32"
          required
        />
            ...
            {selectedFiles.map((file, i) => (
              <div key={i} className="relative w-24 h-24">
                <img
                  ...
                  alt={`preview-${i}`}
                  className="w-full h-full object-cover rounded-xl border border-[#bba987]/50"
                />
                <button
                  type="button"
